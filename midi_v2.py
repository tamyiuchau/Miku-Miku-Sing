...
midi_port='LoopBe Internal MIDI'
pitch_string="""C0	16.35	2109.89
...
B8	7902.13	4.37
""".split()
ref_hz=[float(i) for i in pitch_string[1::3]]
ref=pitch_string[0::3]
def pitch(hz):
...
with open('a.csv','r') as csvfile:
    ...
    t0=0
    next(reader)
    with mido.open_output(midi_port) as mid:
        track=mido.midifiles.MidiTrack()
        mid.append(track)
        track.append(mido.Message('program_change',program=53,time=0.0))
        ...
        for row in reader:
            ...
            if round(a)!=round(b):
                if a:
                    track.append(mido.Message('note_off',note=round(b),
                                               time=float(row[0])-t0))
                track.append(mido.Message('note_off',note=round(b),
                                           time=float(row[0])-t0))
                a=b
